from src.dataset import SpeechDataModule
from src.hifigan_trainer import HifiGANTrainer
...
@hydra.main(config_path='configs', config_name='sparc_hifigan_librittsr')
def main(cfg):
    
    # adjusting relative path to absolute path
    for split in ['train_datasets', 'val_datasets']:
        for dataset_name, path_dict in cfg['data'][split].items():
            for file_name, file_path in path_dict.items():
                cfg['data'][split][dataset_name][file_name] = fix_path(file_path)
     
    # datamodule
    datamodule = SpeechDataModule(**cfg['data'])
    
    # model
    model = HifiGANTrainer(cfg)
