...
from .loss import GeneratorAdversarialLoss, DiscriminatorAdversarialLoss, \
                    MelSpectrogramLoss, FeatureMatchLoss


class HifiGANTrainer(LightningModule):

    def __init__(self, config):
        super().__init__()
        self.config = config
        self.generator = HiFiGANGenerator(**config.generator_configs)
        self.spk_enc = SpeakerEncodingLayer(**config.speaker_encoding_configs)
        self.discriminator = HiFiGANMultiScaleMultiPeriodDiscriminator(**config.discriminator_configs)
    ...
    def forward_gen(self, art, spk, wav, **kwargs):
        #######################
        #      Generator      #
        #######################
        # art : (B, L, d)
        ...
        spkemb = self.spk_enc(spk)
        wav_gen, dur_pred = self.generator(art, spkemb)
        wav = wav.unsqueeze(1)
    ...
    def forward_disc(self, art, spk, wav, **kwargs):
        #######################
        #    Discriminator    #
        #######################
        # art : (B, L, d)
        ...
        with torch.no_grad():
            art = art.transpose(1,2)
            spkemb = self.spk_enc(spk)
            wav_gen, _ = self.generator(ft, spkemb)
        wav = wav.unsqueeze(1)
        gen_disc_pred = self.discriminator(wav_gen)
        gt_disc_pred = self.discriminator(wav)
