...
from sparc.block import HiFiGANResidualBlock as ResidualBlock
from sparc.block import HiFiGANResidualFiLMBlock as ResidualFiLMBlock
...
class HiFiGANScaleDiscriminator(torch.nn.Module):
    ...
    def __init__(
        ...
        # add first layer
        if scale!=1:
            self.layers.append(torch.nn.AvgPool1d(scale, scale))
        self.layers += [
            torch.nn.Sequential(
                torch.nn.Conv1d(
                    in_channels,
...
class HiFiGANMultiScaleDiscriminator(torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(scales):
            ...
            self.discriminators += [HiFiGANScaleDiscriminator(scale = 2**i, **params)]
